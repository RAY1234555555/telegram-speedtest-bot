...
# --- 检查系统环境 ---
log_info "🔍 检查系统环境..."
for cmd in python3 pip3 openssl systemctl curl wget; do
...
sudo_if_needed chown "${CURRENT_USER}:${CURRENT_GROUP}" "/tmp/clash"
...
echo "🚀 启动 IKUN 增强测速机器人 (集成FullTclash)..."
echo "📡 API 地址: \$TELEGRAM_API_URL"
echo "👥 授权用户: \$USER_IDS"
echo "⚡ Clash核心: \$(clash -v 2>/dev/null | head -1 || echo '未安装')"
...
print_header() {
    clear
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║              IKUN增强测速机器人管理面板 (FullTclash)           ║${NC}"
...
show_menu() {
    ...
    echo -e "  ${GREEN}6.${NC} ⚡ 测试Clash"
    echo -e "  ${GREEN}7.${NC} 🗑️  卸载服务"
    echo -e "  ${GREEN}8.${NC} ❌ 退出"
    echo ""
    echo -ne "${CYAN}请输入选项 [1-8]: ${NC}"
}

test_clash() {
    echo -e "${BLUE}⚡ 测试Clash核心...${NC}"
    
    if ! command -v clash >/dev/null 2>&1; then
        echo -e "${RED}❌ Clash核心未安装${NC}"
        read -p "按回车键继续..."
        return
    fi
    
    echo -e "${GREEN}✅ Clash版本: $(clash -v | head -1)${NC}"
    ...
    if netstat -tuln | grep -q ":7890"; then
        echo -e "${YELLOW}⚠️  端口7890已被占用${NC}"
    else
        echo -e "${GREEN}✅ 端口7890可用${NC}"
    fi
    
    if netstat -tuln | grep -q ":9090"; then
        echo -e "${YELLOW}⚠️  端口9090已被占用${NC}"
    else
        echo -e "${GREEN}✅ 端口9090可用${NC}"
    fi
...
show_status() {
    ...
    # Clash状态
    echo -e "${CYAN}=== Clash状态 ===${NC}"
    if command -v clash >/dev/null 2>&1; then
        echo -e "Clash版本: $(clash -v | head -1)"
        echo -e "Clash路径: $(which clash)"
    else
        echo -e "${RED}Clash未安装${NC}"
    fi
    echo ""
...
uninstall_service() {
    echo -e "${RED}⚠️  警告: 这将完全删除IKUN增强测速机器人服务和所有相关文件！${NC}"
    echo ""
    ...
    if [ "$confirm" = "YES" ]; then
        ...
        echo -e "${GREEN}✅ IKUN增强测速机器人已完全卸载！${NC}"
        echo -e "${YELLOW}Clash核心保留在 /usr/local/bin/clash，如需删除请手动执行：${NC}"
        echo -e "${YELLOW}sudo rm -f /usr/local/bin/clash${NC}"
...
[Unit]
Description=IKUN Enhanced Telegram Speed Test Bot with FullTclash
...
TEST_MESSAGE="🎉 IKUN增强测速机器人安装成功！

✅ 服务运行正常
🚀 集成FullTclash核心引擎
⚡ Clash版本: $(clash -v 2>/dev/null | head -1 || echo '未知')
📊 支持真实Clash代理测速
🎬 支持流媒体解锁检测
...
echo "   • FullTclash测速：完整的Clash核心测速"
echo "   • 流媒体解锁：Netflix、Disney+等平台检测"
echo "   • 批量测试：支持多节点并发测试"
echo ""
echo "🚀 现在可以开始使用增强版机器人了！"
